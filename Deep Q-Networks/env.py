...
class ContinuousMazeEnv(gym.Env):
    ...
    def render(self):
        ...
        # Draw danger zone margins (red outline)
            margin = 0.1  
            rect = pygame.Rect(
                (xmin - margin) * self.width,
                self.height - (ymax + margin) * self.height,
                (xmax - xmin + 2 * margin) * self.width,
                (ymax - ymin + 2 * margin) * self.height,
            )
